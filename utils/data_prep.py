...
class h3_grid:
    ...
    def var_continuas(self, ds) -> pd.Series:
        ...
        if isinstance(ds, gpd.GeoDataFrame):
            if ds.crs != gdf_hex.crs:
                ds = ds.to_crs(gdf_hex.crs)
            
            col = ds.columns[0]
            
            # Spatial join polygons to hexagons
            joined = gpd.sjoin(ds[[col, 'geometry']], gdf_hex, how='right')
            
            # Average the numeric column per hexagon
            array = joined.groupby('h3')[col].mean().reindex(gdf_hex['h3'])
            
            return pd.Series(array.values)
        ...
        return pd.Series([s['mean'] for s in stats])
